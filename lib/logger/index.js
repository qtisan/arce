...
var log = function () {
	if (arguments.length == 1) {
		console.log('-----------------------');
		try {
			console.log(JSON.stringify(arguments[0]) + ' - @TIME:' + (new Date()).toLocaleString());
		}
		catch(_e){
			console.log(arguments[0].toString() + ' - @TIME:' + (new Date()).toLocaleString());
		}
	}
	else {
		...
		if (console.hasOwnProperty(method)) {
			console[method].apply(console, [logString + message + filename + ' - @TIME:' + (new Date()).toLocaleString()]);
		}
		else {
			console.log(logString + message + filename + ' - @TIME:' + (new Date()).toLocaleString());
		}
	}
};
