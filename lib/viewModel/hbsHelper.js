...
HbsHelper.prototype.registerQM = function() {
	this.hbs.registerHelper('qm', function(expression, trueValue, falseValue){
...
HbsHelper.prototype.registerSame = function() {
	this.hbs.registerHelper('same', function(exp1, exp2, exp3, context) {
		var ctx = context,
			other = exp3;
		if (typeof exp3 == 'object' && exp3.fn) {
			ctx = exp3;
			other = '';
		}
		return exp1 == exp2 ? (ctx ? ctx.fn(this) : '') : (other || '');
	});
};
...
HbsHelper.prototype.registerStringify = function() {
	this.hbs.registerHelper('stringify', function(object){
		return JSON.stringify(object);
	});
};

HbsHelper.prototype.registerTemplate = function(name, template, options, pre) {
	this.hbs.registerHelper((pre || 'temp_') + name, function() {
		var result = template,
			args = arguments;
...
HbsHelper.prototype.registerWrapper = function(name, template, options, pre) {
	this.hbs.registerHelper((pre || 'wrap_') + name, function() {
		var result = template,
		    args = arguments;
		for (var p in options) {
			if (options.hasOwnProperty(p)) {
				...
				if (typeof opts == 'object') {
					...
					opts = options[p].process(opts.substr(0, opts.length-1));
				}
				result = result.replace('{' + p + '}', opts);
			}
		}
...
HbsHelper.prototype.registerQuery = function() {
	this.hbs.registerHelper('query', function() {
		var pre = arguments[0] ? arguments[0] + '?' : '';
		for (var i = 1; i < arguments.length; i += 2) {
			if (arguments[i + 1]) {
				pre += arguments[i] + '=' + arguments[i+1] + '&';
			}
		}
