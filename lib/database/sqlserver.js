...
//	sqlConnection.on('infoMessage', function(info) {
//		console.log('---- info message:');
//		console.log(info);
//	});
//	sqlConnection.on('charsetChange', function(chatset){
//		console.log('---- chatset change:');
//		console.log(chatset);
//	});
//	sqlConnection.on('languageChange', function(languageName){
//		console.log('---- language change:');
//		console.log(languageName);
//	});
//	sqlConnection.on('debug', function(text) {
//		console.log('---- debug:');
//		console.log(text);
//	});
...
SqlServer.prototype.callProcedure = function (request) {
	if(this._conn.state.name != 'LoggedIn') {
		var self = this;
		console.warn('||| blocked...', request.sqlTextOrProcedure +
			' - @WITH_PARAM: '+JSON.stringify(request.parameters), __filename); // TODO: log
		setTimeout(function(){
			self.callProcedure(request);
		}, 100);
	}
	else {
		console.log('>>> my state: <| ' + request.sqlTextOrProcedure + ' |> ->' +
			this._conn.state.name + ' - @WITH_PARAM: '+JSON.stringify(request.parameters)); // TODO: log
